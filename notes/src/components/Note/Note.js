...
const Note = (props) => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:3001/items?noteId=${id}`)
      .then((res) => {
        setItems(res.data);
      })
      .catch((err) => {
  ...
  const renderItems = (items) => {
    return items.map((item) => {
      return <Item data={item} refresh={refreshToNote} />;
    });
  };
    ...
    <div class="ui card note">
      <div class="content">
        <div class="header note-header">
          {isEditing && (
            <>
              <div class="field" onBlur={() => setIsEditing(false)}>
                <input
                ...
                >
                  <i class="save icon" style={{ fontSize: "1.3rem" }}></i>
                </button>
              </div>
            </>
          )}
          {!isEditing && (
            <>
              <span onDoubleClick={() => editNote()} class="left floated title">
                {title}
              </span>
      ...
      <div class="extra content">
        ...
        <span
          ...
          class="right floated delete note-delete"
        >
